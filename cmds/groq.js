...
// ملاحظة إصلاح مهمة: كان هذا الاستدعاء يرسل { messages } فقط، وكانت بيانات
// المرفق (attachment) موضوعة داخل آخر عنصر في messages (userMsg.attachment).
// لكن الباك-إند (plugins/groq/groq.go) يقرأ المرفق حصراً من حقل جذر منفصل
// body["attachment"] عبر parseMessages()/parseAttachment() — وهذا الحقل لم
// يكن يصل إطلاقاً، فكانت ميزة تحليل الصورة/الصوت/الفيديو تتحول بصمت لمحادثة
// نصية عادية بدون أي خطأ ظاهر. الإصلاح: نرسل المرفق أيضاً كحقل جذر مطابق
// تماماً لما يقرأه الخادم.
...
async function handle(api, event, prompt, registerReply) {
  ...
  let reply;
  try {
    reply = await callHF(messages, userMsg.attachment);
  } catch (e) {
...
export default {
  config: {
    ...
    description: "محادثة ذكية جماعية مع دعم الرؤية (صورة/صوت/فيديو) — Llama 4 Scout",
    usage: [
