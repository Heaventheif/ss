...
// كل الـ APIs المتاحة — كل واحد يرسم صورة تفاعل (صفع) بين شخصين بأسلوب مختلف.
// كل API له أسماء بارامترات مختلفة للمعرفين، لذلك نحتفظ بها هنا.
// الترتيب هنا هو ترتيب "دائرة التشغيل" (round-robin): كل استدعاء لـ slap
// يستخدم العنصر التالي في هذه القائمة، وعند الوصول للنهاية يرجع للبداية.
const SLAP_APIS = [
...
// Get the next API in the rotation and advance the shared counter.
...
export default {
  ...
  // Command entry point: pick the next slap style in rotation, resolve the
  // two target UIDs (sender + target), fetch the generated image, and send it.
  onStart: async ({ api, event, args, message, globalData }) => {
    ...
    // id1 = من قام بالصفع (مُرسل الأمر)
    const id1 = senderID;

    // id2 = الشخص المستهدف (رد / منشن / UID مكتوب)
    let id2;
