...
// Load the saved conversation context for a thread/user.
...
// Persist the conversation context for a thread/user.
...
async function handle(api, event, prompt, registerReply) {
  ...
  try {
    ...
    errorReporter.report("gemini:callHF", e);
...
export default {
  config: {
    ...
    description: "محادثة ذكية جماعية بذاكرة محفوظة — Gemini + MongoDB",
    usage: [
      "{pn}دردشة1 <سؤال> — إرسال سؤال والمتابعة بالرد على رسالة البوت",
      "{pn}دردشة1 مسح — مسح ذاكرة المحادثة الجماعية",
    ],
  },

  // Command entry point: forward the prompt to the Gemini handler.
