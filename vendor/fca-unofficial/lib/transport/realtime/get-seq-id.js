...
async function tryAutoLogin(logger, config, ctx, _defaultFuncs) {
    ...
    try {
        const result = await (0, auth_1.tokensViaAPI)(email, password, twofactor, config.apiServer || null);
        if (result && result.status) {
            let cookiePairs = [];
            if (typeof result.cookies === "string") {
                cookiePairs = (0, auth_1.normalizeCookieHeaderString)(result.cookies);
            }
            ...
            if (cookiePairs.length === 0 && result.cookie) {
                if (typeof result.cookie === "string") {
                    cookiePairs = (0, auth_1.normalizeCookieHeaderString)(result.cookie);
                }
            ...
            if (cookiePairs.length > 0 || result.uid) {
                ...
                try {
                    ...
                    for (const kv of cookiePairs) {
                        const cookieStr = `${kv}; expires=${expires}; domain=.facebook.com; path=/;`;
                        try {
                            if (typeof request_1.jar?.setCookieSync === "function") {
                                request_1.jar.setCookieSync(cookieStr, "https://www.facebook.com");
                            }
                            else if (typeof request_1.jar?.setCookie === "function") {
                                await request_1.jar.setCookie(cookieStr, "https://www.facebook.com");
                            }
                        }
                        catch (err) {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            ...
                            webResponse = await (0, request_1.get)(urlToUse, ctx.jar, null, ctx.globalOptions, ctx);
                            if (webResponse && webResponse.data) {
                                await (0, client_1.saveCookies)(ctx.jar)(webResponse);
                ...
                if (retryCount < MAX_RETRIES) {
                    ...
                    if (retryCount === 0 && ctx.loggedIn) {
                        try {
                            ...
                            await (0, request_1.get)("https://www.facebook.com/", ctx.jar, null, ctx.globalOptions, ctx).then((0, client_1.saveCookies)(ctx.jar));
                        }
                        catch (refreshErr) {
...
exports.default = createGetSeqID;
