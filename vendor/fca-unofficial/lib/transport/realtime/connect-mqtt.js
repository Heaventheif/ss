...
function createListenMqtt(deps) {
    const { WebSocket, mqtt, HttpsProxyAgent, buildStream, buildProxy, topics, parseDelta, getTaskResponseData, logger, emitAuth } = deps;
    return function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
        ...
        mqttClient.on("message", function (topic, message) {
            if (ctx._ending || ctx.mqttClient !== mqttClient)
                return;
            try {
                ...
                if (jsonMessage.type === "jewel_requests_add") {
                    ...
                    if (tasks && tasks instanceof Map && tasks.has(reqID)) {
                        ...
                        if (taskRespData == null)
                            ...
                            taskCallback(null, Object.assign({ type: taskType, reqID }, taskRespData));
                    }
                }
            }
            catch (ex) {
...
module.exports = createListenMqtt;
