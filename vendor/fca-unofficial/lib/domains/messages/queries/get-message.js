...
exports.createGetMessageQuery = createGetMessageQuery;
const format_1 = __importDefault(format);
const { _formatAttachment } = format_1.default;
function formatThemeMetadata(data) {
    return {
...
function formatBlobAttachments(blobAttachments) {
    return blobAttachments.map((attachment) => {
        try {
...
function formatExtensibleAttachment(extensibleAttachment) {
                        ...
                        logMessageData: {
                            question_json: JSON.stringify({
                                ...
                                options: Array.isArray(question?.options?.nodes)
                                    ? question.options.nodes.map((option) => ({
                                        id: option.id,
                                        text: option.text,
                                        ...
                                        voters: Array.isArray(option.voters?.nodes)
                                            ? option.voters.nodes.map((voter) => voter.id)
                                            : []
                                    }))
                                    : []
                            }),
...
function createGetMessageQuery(deps) {
...
const __export_createGetMessageQuery = module.exports.createGetMessageQuery;
export { __export_createGetMessageQuery as createGetMessageQuery };
