...
const { formatDeltaEvent, formatMessage, _formatAttachment, formatDeltaMessage, formatDeltaReadReceipt, formatID, getType, decodeClientPayload, getMentionsFromDeltaMessage } = format_1.default;
function emitThreadInfoEvent(ctx, globalCallback, payload) {
    globalCallback(null, payload);
    if (payload &&
        payload.type === "event" &&
        payload.threadID != null &&
        typeof ctx._syncThreadInfoFromEvent === "function") {
        try {
            ctx._syncThreadInfoFromEvent(payload);
        }
        catch {
            /* ignore */
        }
...
function createParseDelta(deps) {
    ...
    return function parseDelta(defaultFuncs, api, ctx, globalCallback, { delta }) {
                    ...
                    if (d.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
                        ...
                        try {
                            ...
                            if (!msg || !msg.messageMetadata) {
                                (0, logger_1.default)("parseDelta: deltaMessageReply.message or messageMetadata is missing", "warn");
                                return;
                            }
                            ...
                            callbackToReturn = {
                                ...
                                attachments: (msg.attachments || []).map((att) => {
                                    ...
                                    let x;
                                    try {
                                        x = _formatAttachment(att, undefined);
                                    }
                                    catch (ex) {
                                        x = att;
                                        x.error = ex;
                            ...
                            if (d.deltaMessageReply.repliedToMessage) {
                                try {
                                    ...
                                    (0, logger_1.default)(`parseDelta message_reply repliedToMessage error: ${errMsg}`, "warn");
                                }
                            }
                                ...
                                return defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, {
                                    ...
                                    callbackToReturn.messageReply = {
                                        ...
                                        attachments: fetchData.message.blob_attachment.map((att) => _formatAttachment({ blob_attachment: att }, undefined)),
                                    ...
                                    const errMsg = err && err.message ? err.message : String(err || "Unknown error");
                                    (0, logger_1.default)(`parseDelta message_reply fetch error: ${errMsg}`, "warn");
                                }).finally(() => {
                                    if (callbackToReturn) {
                            ...
                            const errMsg = err && err.message ? err.message : String(err || "Unknown error");
                            (0, logger_1.default)(`parseDelta message_reply error: ${errMsg}`, "warn");
                            return;
                        }
                        if (callbackToReturn) {
                            if (!ctx.globalOptions.selfListen && callbackToReturn.senderID === ctx.userID)
        ...
        switch (delta.class) {
            ...
            case "ForcedFetch": {
                ...
                if (mid && tid) {
                    ...
                    defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form).then(parseAndCheckLogin(ctx, defaultFuncs)).then((resData) => {
                        ...
                        if (getType(fetchData) === "Object") {
                            switch (fetchData.__typename) {
                                case "ThreadImageMessage":
                                    if ((!ctx.globalOptions.selfListen && fetchData.message_sender.id.toString() === ctx.userID) || !ctx.loggedIn) { }
                                    else {
                                        emitThreadInfoEvent(ctx, globalCallback, {
                                            type: "event",
                        ...
                        const errMsg = err && err.message ? err.message : String(err || "Unknown error");
                        (0, logger_1.default)(`parseDelta ForcedFetch error: ${errMsg}`, "warn");
                    });
                }
                break;
            }
            ...
            case "ParticipantLeftGroupThread": {
                ...
                if (!ctx.loggedIn)
                    return;
                emitThreadInfoEvent(ctx, globalCallback, formattedEvent);
                break;
            }
            case "NewMessage": {
...
module.exports = createParseDelta;
